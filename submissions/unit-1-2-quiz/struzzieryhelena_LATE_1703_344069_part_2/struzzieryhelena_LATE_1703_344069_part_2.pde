...
void setup() {
  ...
  x = width/2;
  y = height/2;
  xspeed = 5; // Choose a number between 1-5
  yspeed = 5; // Choose a number between 1-5
  size1 = 50;
  size2 = 25;
  // 1/1
}
...
void draw() {
  ...
  rect(0, 0, 500, 500); // .5/1 wanted you to use height and width variables

  // Update positions using speed variables
  x = 2;
  y = 2;
  // 0/2 need to update using something like x = x + xspeed or x += xspeed
...
void mousePressed() {
  ...
  size1 = random(20, 80);
  size2 = random(30, 90); // should be random(10, 100)
  
  // Give speed new random values between 1 and 5
  xspeed = random(2); 
  yspeed = random(2);  // should be random (1, 5)
  // 2.5/4
}
